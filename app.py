...
def q_pdf(q,client):
    ...
    mayoristas = ParagraphStyle("RLMayo", parent=body, fontName="Helvetica-Bold", fontSize=10, leading=11, textColor=green, alignment=1)
    ...
    # Brand block: color PNG + MAYORISTAS centered beneath it at the same visual width.
    if LOGO_PATH.exists():
        logo = Image(str(LOGO_PATH), width=62*mm, height=41*mm, kind="proportional")
        brand_block = [logo, Spacer(1, 1.2*mm), Paragraph("MAYORISTAS", mayoristas)]
    else:
        brand_block = [Paragraph("<b>Raíz Latina</b>", section), Paragraph("MAYORISTAS", mayoristas)]
