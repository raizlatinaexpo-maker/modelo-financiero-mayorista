...
def read_sql_query(query, conn_obj=None, params=None):
    """Execute a SELECT through the PostgreSQL connection used by the app.
    conn_obj is accepted for compatibility with the existing calls throughout the app.
    """
    db = conn_obj.raw if conn_obj is not None else conn.raw
    return pd.read_sql_query(query.replace("?", "%s"), db, params=params)
...
def q_next_number():
    row=conn.execute("SELECT quotation_number FROM quotations ORDER BY id DESC LIMIT 1").fetchone()
...
def q_client(client_id):
    r=conn.execute("SELECT id,name,country,city,address,postal_code,email,phone FROM clients WHERE id=?",(int(client_id),)).fetchone()
...
def q_pdf(q,client):
    ...
    buf=BytesIO(); doc=SimpleDocTemplate(buf,pagesize=A4,leftMargin=15*mm,rightMargin=15*mm,topMargin=12*mm,bottomMargin=12*mm)
    styles=getSampleStyleSheet(); green=colors.HexColor("#1B4D3E"); cream=colors.HexColor("#F5F1E9"); dark=colors.HexColor("#303633"); gray=colors.HexColor("#6C726E")
    body=ParagraphStyle("RLBody",parent=styles["BodyText"],fontName="Helvetica",fontSize=9,leading=11,textColor=dark)
    small=ParagraphStyle("RLSmall",parent=body,fontSize=7.8,leading=9.5)
    title=ParagraphStyle("RLTitle",parent=body,fontName="Helvetica-Bold",fontSize=20,textColor=green)
    head=ParagraphStyle("RLHead",parent=body,fontName="Helvetica-Bold",fontSize=9,textColor=green)
    story=[]
    logo=Image(str(LOGO_PATH),width=47*mm,height=25*mm,kind="proportional") if LOGO_PATH.exists() else Paragraph("<b>Raíz Latina</b>",title)
    meta=Paragraph(f"<b>Cotización:</b> {q['quotation_number']}<br/><b>Fecha:</b> {q['created_at']}<br/><b>Cotización válida hasta:</b> {q['valid_until']}",small)
    t=Table([[logo,meta]],colWidths=[100*mm,70*mm]); t.setStyle(TableStyle([("VALIGN",(0,0),(-1,-1),"MIDDLE"),("ALIGN",(1,0),(1,0),"RIGHT")]))
    story += [t,Spacer(1,4*mm),Paragraph("COTIZACIÓN",title),Spacer(1,4*mm)]
    address="<br/>".join([str(x) for x in [client.get("address"),client.get("city"),client.get("postal_code"),client.get("country")] if str(x or "").strip()])
    dest=f"<b>{client.get('name','')}</b><br/>{address}"
    if client.get("phone"): dest += f"<br/>{client['phone']}"
    if client.get("email"): dest += f"<br/>{client['email']}"
    info=Table([[Paragraph("<b>DATOS DE RAÍZ LATINA</b><br/>Medellín, Colombia",body),Paragraph("<b>DATOS DEL CLIENTE</b><br/>"+dest,body)]],colWidths=[85*mm,85*mm])
    info.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1),cream),("BOX",(0,0),(-1,-1),0.4,colors.HexColor("#D8D2C6")),("VALIGN",(0,0),(-1,-1),"TOP"),("LEFTPADDING",(0,0),(-1,-1),5*mm),("RIGHTPADDING",(0,0),(-1,-1),5*mm),("TOPPADDING",(0,0),(-1,-1),4*mm),("BOTTOMPADDING",(0,0),(-1,-1),4*mm)]))
    story += [info,Spacer(1,5*mm)]
    rows=[["CONCEPTO",f"VALOR ({q['currency']})"],["Valor productos",q_money(q["products_amount"],q["currency"])]]
    if q["product_discount_amount"]>0: rows.append(["Descuento productos"+(f" ({q['product_discount_value']:.0f}%)" if q["product_discount_type"]=="%" else ""),"- "+q_money(q["product_discount_amount"],q["currency"])])
    rows.append(["Tarifa de envío",q_money(q["shipping_base"],q["currency"])])
    if q["shipping_discount_amount"]>0: rows.append(["Descuento envío"+(f" ({q['shipping_discount_value']:.0f}%)" if q["shipping_discount_type"]=="%" else ""),"- "+q_money(q["shipping_discount_amount"],q["currency"])])
    if q["tax_amount"]>0: rows.append(["Impuestos",q_money(q["tax_amount"],q["currency"])])
    rows.append(["TOTAL",q_money(q["total_amount"],q["currency"])])
    tab=Table(rows,colWidths=[112*mm,58*mm]); tab.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),green),("TEXTCOLOR",(0,0),(-1,0),colors.white),("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),("FONTNAME",(0,-1),(-1,-1),"Helvetica-Bold"),("BACKGROUND",(0,-1),(-1,-1),colors.HexColor("#E7ECD9")),("ALIGN",(1,1),(1,-1),"RIGHT"),("GRID",(0,0),(-1,-1),0.35,colors.HexColor("#C8C8C8")),("FONTSIZE",(0,0),(-1,-1),8.7),("TOPPADDING",(0,0),(-1,-1),4),("BOTTOMPADDING",(0,0),(-1,-1),4)]))
    story += [tab,Spacer(1,4*mm)]
    info2=Table([[Paragraph(f"<b>Peso facturado:</b> {q['billed_weight']:.1f} kg<br/><b>País de destino:</b> {q['country']}<br/><b>Término de negociación:</b> {q['incoterm']}",body)]],colWidths=[170*mm])
    info2.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1),cream),("BOX",(0,0),(-1,-1),0.4,colors.HexColor("#D8D2C6")),("LEFTPADDING",(0,0),(-1,-1),5*mm),("TOPPADDING",(0,0),(-1,-1),4*mm)]))
    story += [info2,Spacer(1,4*mm)]
    cond="<b>CONDICIONES COMERCIALES</b><br/>• Esta cotización es válida por 15 días calendario desde su fecha de creación.<br/>• El valor de impuestos deberá ser pagado por el cliente a la transportadora según las condiciones de cada país.<br/>• En DDP, los impuestos incluidos son únicamente los indicados expresamente en esta cotización.<br/>• Los tiempos de entrega pueden variar según el país de destino."
    ct=Table([[Paragraph(cond,small)]],colWidths=[170*mm]); ct.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1),cream),("BOX",(0,0),(-1,-1),0.4,colors.HexColor("#D8D2C6")),("LEFTPADDING",(0,0),(-1,-1),5*mm),("RIGHTPADDING",(0,0),(-1,-1),5*mm),("TOPPADDING",(0,0),(-1,-1),4*mm),("BOTTOMPADDING",(0,0),(-1,-1),4*mm)]))
    story += [ct,Spacer(1,3*mm),Paragraph("Medellín, Colombia",small)]
...
def render_quotations():
    ...
    with tab1:
        ...
        if clients.empty: st.warning("Primero crea al menos un cliente en Clientes.")
        else:
            ids=clients["id"].tolist(); sel=st.selectbox("Buscar cliente",ids,format_func=lambda x:f"{clients.loc[clients.id==x,'name'].iloc[0]} — {clients.loc[clients.id==x,'country'].iloc[0] or 'Sin país'}")
            c=q_client(sel)
            a,b=st.columns(2)
            with a:
                name=st.text_input("Nombre / Empresa",value=c["name"] or ""); email=st.text_input("Correo",value=c["email"] or ""); phone=st.text_input("Teléfono",value=c["phone"] or ""); country=st.text_input("País",value=c["country"] or "")
            with b:
                city=st.text_input("Ciudad",value=c["city"] or ""); address=st.text_area("Dirección",value=c["address"] or ""); postal=st.text_input("Código postal",value=c["postal_code"] or "")
            x,y,z=st.columns(3)
            with x:
                cur_default=q_country_currency(country); currency=st.selectbox("Divisa",["EUR","USD"],index=0 if cur_default=="EUR" else 1); products=st.number_input("Valor de productos",min_value=0.0,step=10.0); weight=st.number_input("Peso de productos (kg)",min_value=0.0,step=0.5)
            with y:
                pdt=st.selectbox("Descuento productos",["NINGUNO","%","VALOR"]); pdv=st.number_input("Descuento productos",min_value=0.0,step=1.0,disabled=pdt=="NINGUNO"); inc=st.selectbox("Término de negociación",["DAP","DDP"])
            with z:
                sdt=st.selectbox("Descuento envío",["NINGUNO","%","VALOR"]); sdv=st.number_input("Descuento envío",min_value=0.0,step=1.0,disabled=sdt=="NINGUNO"); st.caption("En DDP: 35 por caja por defecto; puede modificarse.")
            if st.button("🧮 Calcular cotización",type="primary"):
                calc,err=q_calculate_shipping(country,weight)
                if err: st.error(err)
                elif products<=0: st.error("Ingresa el valor de productos.")
                else:
                    pdisc=q_discount(products,pdt,pdv); sdisc=q_discount(calc["total"],sdt,sdv); pnet=products-pdisc; snet=calc["total"]-sdisc; tax=calc["boxes"]*35 if inc=="DDP" else 0; total=pnet+snet+tax
                    st.session_state["quote_draft"]={"client_id":sel,"name":name,"email":email,"phone":phone,"country":country,"city":city,"address":address,"postal_code":postal,"currency":currency,"products_amount":products,"product_discount_type":pdt,"product_discount_value":pdv,"product_discount_amount":pdisc,"product_net":pnet,"product_weight":weight,"billed_weight":calc["billed_weight"],"boxes_count":calc["boxes"],"shipping_base":calc["total"],"shipping_discount_type":sdt,"shipping_discount_value":sdv,"shipping_discount_amount":sdisc,"shipping_net":snet,"incoterm":inc,"tax_amount":tax,"tax_per_box":35.0,"tax_manual":False,"total_amount":total,"box_details":calc["details"],"notes":""}
                    st.rerun()
            qd=st.session_state.get("quote_draft")
            if qd:
                st.divider(); st.subheader("Resultado de la cotización")
                m1,m2,m3,m4=st.columns(4); m1.metric("Cajas",qd["boxes_count"]); m2.metric("Peso facturado",f"{qd['billed_weight']:.1f} kg"); m3.metric("Envío",q_money(qd["shipping_net"],qd["currency"])); m4.metric("Total",q_money(qd["total_amount"],qd["currency"]))
                with st.expander("Información interna de cajas"):
                    for d in qd["box_details"]: st.write(f"Caja {d['box']}: {d['product_kg']:.2f} kg productos + 1 kg embalaje = {d['billed_kg']:.1f} kg facturados · tarifa {q_money(d['rate'],qd['currency'])}")
                if qd["incoterm"]=="DDP":
                    ...
                    qd["tax_amount"]=tax; qd["tax_manual"]=manual; qd["total_amount"]=qd["product_net"]+qd["shipping_net"]+tax
                ...
                if st.button("💾 Guardar cotización",type="primary"):
                    ...
                    conn.execute("""INSERT INTO quotations(quotation_number,client_id,recipient_name,created_at,valid_until,country,city,address,postal_code,email,phone,currency,products_amount,product_discount_type,product_discount_value,product_discount_amount,product_net,product_weight,packaging_weight,billed_weight,boxes_count,shipping_base,shipping_discount_type,shipping_discount_value,shipping_discount_amount,shipping_net,incoterm,tax_amount,tax_per_box,tax_manual,total_amount,notes,status,box_details) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",(num,sel,qd["name"],str(today),str(valid),qd["country"],qd["city"],qd["address"],qd["postal_code"],qd["email"],qd["phone"],qd["currency"],qd["products_amount"],qd["product_discount_type"],qd["product_discount_value"],qd["product_discount_amount"],qd["product_net"],qd["product_weight"],qd["boxes_count"],qd["billed_weight"],qd["boxes_count"],qd["shipping_base"],qd["shipping_discount_type"],qd["shipping_discount_value"],qd["shipping_discount_amount"],qd["shipping_net"],qd["incoterm"],qd["tax_amount"],qd["tax_per_box"],qd["tax_manual"],qd["total_amount"],qd["notes"],"PENDIENTE",str(qd["box_details"])))
                    conn.commit(); st.session_state.pop("quote_draft",None); st.success(f"{num} guardada. Puedes verla en Listado."); st.rerun()
    with tab2:
        df=read_sql_query("""SELECT q.id,q.quotation_number AS \"Cotización\",q.created_at AS \"Fecha\",c.name AS \"Cliente\",q.country AS \"País\",q.total_amount AS \"Total\",q.currency AS \"Divisa\",q.status AS \"Estado\" FROM quotations q JOIN clients c ON c.id=q.client_id ORDER BY q.id DESC""",conn)
        if df.empty: st.info("Todavía no hay cotizaciones.")
        else:
            st.dataframe(df.drop(columns=["id"]),use_container_width=True,hide_index=True)
            qid=st.selectbox("Selecciona una cotización",df["id"].astype(int).tolist(),format_func=lambda x:f"{df.loc[df.id==x,'Cotización'].iloc[0]} — {df.loc[df.id==x,'Cliente'].iloc[0]}",key="quote_selector")
            r=conn.execute("SELECT id,quotation_number,client_id,recipient_name,created_at,valid_until,country,city,address,postal_code,email,phone,currency,products_amount,product_discount_type,product_discount_value,product_discount_amount,product_net,product_weight,packaging_weight,billed_weight,boxes_count,shipping_base,shipping_discount_type,shipping_discount_value,shipping_discount_amount,shipping_net,incoterm,tax_amount,tax_per_box,tax_manual,total_amount,notes,status,order_id,box_details FROM quotations WHERE id=?",(int(qid),)).fetchone()
            if r:
                keys=["id","quotation_number","client_id","recipient_name","created_at","valid_until","country","city","address","postal_code","email","phone","currency","products_amount","product_discount_type","product_discount_value","product_discount_amount","product_net","product_weight","packaging_weight","billed_weight","boxes_count","shipping_base","shipping_discount_type","shipping_discount_value","shipping_discount_amount","shipping_net","incoterm","tax_amount","tax_per_box","tax_manual","total_amount","notes","status","order_id","box_details"]
                q=dict(zip(keys,r)); client=q_client(q["client_id"])
                if client:
                    client = client.copy()
                ...
                if q["notes"]: st.info("Observación: "+q["notes"])
                # PDF always reflects current saved values.
                if REPORTLAB_OK and client:
                    safe=re.sub(r"[^A-Za-z0-9ÁÉÍÓÚáéíóúÑñ _-]+","",client["name"] or "cliente").strip().replace(" ","_")
                    pdf=q_pdf(q,client)
                    st.download_button("📄 Descargar PDF",pdf,f"Cotizacion {q['quotation_number']} - {safe}.pdf","application/pdf",key=f"pdf_{qid}")
                st.divider(); st.subheader("Editar cotización")
                editable=q["status"]!="CONFIRMADA"
                if not editable: st.info("Esta cotización ya fue confirmada y convertida en pedido. No se modifica desde aquí.")
                ea,eb=st.columns(2)
                with ea:
                    ename=st.text_input("Nombre / Empresa",value=q["recipient_name"] or client["name"],key=f"ename_{qid}")
                    ecountry=st.text_input("País",value=q["country"],key=f"ecountry_{qid}"); ecity=st.text_input("Ciudad",value=q["city"] or "",key=f"ecity_{qid}"); eaddress=st.text_area("Dirección",value=q["address"] or "",key=f"eaddress_{qid}")
                with eb:
                    eemail=st.text_input("Correo",value=q["email"] or "",key=f"eemail_{qid}"); ephone=st.text_input("Teléfono",value=q["phone"] or "",key=f"ephone_{qid}"); epostal=st.text_input("Código postal",value=q["postal_code"] or "",key=f"epostal_{qid}")
                ec1,ec2,ec3=st.columns(3)
                with ec1: ecurrency=st.selectbox("Divisa",["EUR","USD"],index=0 if q["currency"]=="EUR" else 1,key=f"ecur_{qid}"); eproducts=st.number_input("Valor productos",min_value=0.0,value=float(q["products_amount"]),step=10.0,key=f"eprod_{qid}"); epweight=st.number_input("Peso productos (kg)",min_value=0.0,value=float(q["product_weight"]),step=0.5,key=f"epw_{qid}")
                with ec2: epdt=st.selectbox("Descuento productos",["NINGUNO","%","VALOR"],index=["NINGUNO","%","VALOR"].index(q["product_discount_type"]),key=f"epdt_{qid}"); epdv=st.number_input("Valor descuento productos",min_value=0.0,value=float(q["product_discount_value"]),step=1.0,disabled=epdt=="NINGUNO",key=f"epdv_{qid}"); einc=st.selectbox("Término",["DAP","DDP"],index=0 if q["incoterm"]=="DAP" else 1,key=f"einc_{qid}")
                with ec3: esdt=st.selectbox("Descuento envío",["NINGUNO","%","VALOR"],index=["NINGUNO","%","VALOR"].index(q["shipping_discount_type"]),key=f"esdt_{qid}"); esdv=st.number_input("Valor descuento envío",min_value=0.0,value=float(q["shipping_discount_value"]),step=1.0,disabled=esdt=="NINGUNO",key=f"esdv_{qid}")
                if einc=="DDP":
                    emod=st.checkbox("Modificar manualmente impuestos",value=bool(q["tax_manual"]),key=f"emod_{qid}"); etax=st.number_input("Impuestos",min_value=0.0,value=float(q["tax_amount"]),step=1.0,key=f"etax_{qid}") if emod else float(epweight and q["tax_per_box"]*q["boxes_count"])
                else: emod=False; etax=0.0
                ...
                if editable and st.button("💾 Guardar cambios y recalcular",type="primary",key=f"saveq_{qid}"):
                    ...
                    if err: st.error(err)
                        ...
                        pdisc=q_discount(eproducts,epdt,epdv); sdisc=q_discount(calc["total"],esdt,esdv); pnet=eproducts-pdisc; snet=calc["total"]-sdisc; tax=(calc["boxes"]*35 if einc=="DDP" and not emod else etax); total=pnet+snet+tax; valid=date.fromisoformat(q["created_at"])+timedelta(days=15)
                        conn.execute("""UPDATE quotations SET recipient_name=?,country=?,city=?,address=?,postal_code=?,email=?,phone=?,currency=?,products_amount=?,product_discount_type=?,product_discount_value=?,product_discount_amount=?,product_net=?,product_weight=?,packaging_weight=?,billed_weight=?,boxes_count=?,shipping_base=?,shipping_discount_type=?,shipping_discount_value=?,shipping_discount_amount=?,shipping_net=?,incoterm=?,tax_amount=?,tax_per_box=?,tax_manual=?,total_amount=?,notes=?,valid_until=?,box_details=? WHERE id=?""",(ename,ecountry,ecity,eaddress,epostal,eemail,ephone,ecurrency,eproducts,epdt,epdv,pdisc,pnet,epweight,calc["boxes"],calc["billed_weight"],calc["boxes"],calc["total"],esdt,esdv,sdisc,snet,einc,tax,35.0,emod,total,enotes,str(valid),str(calc["details"]),int(qid)))
                ...
                if q["status"]=="PENDIENTE":
                    ...
                    with a1:
                        if st.button("✅ Confirmar y crear pedido",key=f"confirm_{qid}"):
                            st.session_state["prefill_order"]={"quotation_id":q["id"],"client_id":q["client_id"],"country":q["country"],"currency":q["currency"],"products_amount":q["product_net"],"shipping_amount":q["shipping_net"],"incoterm":q["incoterm"],"tax_amount":q["tax_amount"],"tax_per_box":q["tax_per_box"],"boxes_count":q["boxes_count"],"billed_weight":q["billed_weight"],"notes":q["notes"]}; st.session_state["_goto_page"]="Nuevo pedido"; st.rerun()
                    with a2:
                        if st.button("🗑️ Descartar cotización",key=f"discard_{qid}"):
                            conn.execute("UPDATE quotations SET status='DESCARTADA' WHERE id=?",(int(qid),)); conn.commit(); st.rerun()
    with tab3:
        ...
        if up and st.button("Actualizar tarifas",type="primary"):
            try:
                n=q_import_rates(up); st.success(f"{n} tarifas actualizadas."); st.rerun()
            except Exception as e: st.error(f"No se pudieron actualizar las tarifas: {e}")
        rates=q_load_rates()
...
# ---------------- ORDER DETAILS / OBSERVATIONS ----------------
    if not df.empty:
        ...
        detail = conn.execute("SELECT notes, incoterm, tax_amount_original, tax_currency FROM orders WHERE id=?", (int(detail_id),)).fetchone()
        if detail:
            a,b,c=st.columns(3)
