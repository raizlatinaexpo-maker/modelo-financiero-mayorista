...
def q_pdf(q,client):
    """Generate a polished, white-background international quotation PDF."""
    if not REPORTLAB_OK:
        return None
    buf = BytesIO()
    ...
    dark = colors.HexColor("#222824")
    gray = colors.HexColor("#69716C")
    line = colors.HexColor("#C9D1CC")
    yellow = colors.HexColor("#F6FF00")

    body = ParagraphStyle("RLBody", parent=styles["BodyText"], fontName="Helvetica", fontSize=9, leading=11, textColor=dark)
    small = ParagraphStyle("RLSmall", parent=body, fontSize=7.7, leading=9.5, textColor=gray)
    section = ParagraphStyle("RLSection", parent=body, fontName="Helvetica-Bold", fontSize=9.2, leading=11, textColor=green)
    title = ParagraphStyle("RLTitle", parent=body, fontName="Helvetica-Bold", fontSize=19, leading=21, textColor=green, alignment=2)
    quote_code = ParagraphStyle("RLCode", parent=body, fontName="Helvetica-Bold", fontSize=11.5, leading=14, textColor=dark, alignment=2)
    meta = ParagraphStyle("RLMeta", parent=body, fontSize=8.3, leading=11, textColor=gray, alignment=2)
    total_style = ParagraphStyle("RLTotal", parent=body, fontName="Helvetica-Bold", fontSize=12.5, leading=14, textColor=dark, alignment=2)

    story = []

    # Brand block: the color logo already contains MAYORISTAS integrated into the same artwork.
    if LOGO_PATH.exists():
        logo = Image(str(LOGO_PATH), width=62*mm, height=45.7*mm, kind="proportional")
        brand_block = [logo]
    else:
        brand_block = [Paragraph("<b>Raíz Latina</b>", section)]

    right_block = [
        Paragraph("COTIZACIÓN INTERNACIONAL", title),
        Spacer(1, 1.5*mm),
        Paragraph(q["quotation_number"], quote_code),
        Spacer(1, 2*mm),
        Paragraph(f"Fecha de creación: {q['created_at']}", meta),
        Paragraph(f"Válida hasta: {q['valid_until']}", meta),
    ]
    header = Table([[brand_block, right_block]], colWidths=[95*mm, 75*mm])
    ...
    # Clean divider, then seller/client information with no boxes.
    divider = Table([[""]], colWidths=[170*mm], rowHeights=[0.7*mm])
    divider.setStyle(TableStyle([("BACKGROUND", (0,0), (-1,-1), green), ("LINEBELOW", (0,0), (-1,-1), 0, green)]))
    story += [divider, Spacer(1, 4.5*mm)]
    ...
    for label, value in [
        ...
        if value:
            client_lines.append(f"{label + ': ' if label else ''}{value}")
    ...
    info = Table([
        [Paragraph(seller_text, body), Paragraph("<b>Datos del cliente</b><br/>" + client_text, body)]
    ...
    story += [info, Spacer(1, 5.5*mm)]

    # Financial summary: only the concepts requested by the user.
    rows = [["CONCEPTO", f"VALOR ({q['currency']})"]]
    rows.append(["Valor Productos", q_money(q["products_amount"], q["currency"])])
    if q["product_discount_amount"] > 0:
        label = "Descuento productos"
        if q["product_discount_type"] == "%":
            label += f" ({q['product_discount_value']:.0f}%)"
        rows.append([label, "- " + q_money(q["product_discount_amount"], q["currency"])])
    rows.append(["Valor envío", q_money(q["shipping_base"], q["currency"])])
    if q["shipping_discount_amount"] > 0:
        label = "Descuento envío"
        if q["shipping_discount_type"] == "%":
            label += f" ({q['shipping_discount_value']:.0f}%)"
        rows.append([label, "- " + q_money(q["shipping_discount_amount"], q["currency"])])
    if q["tax_amount"] > 0:
        rows.append(["Valor impuestos", q_money(q["tax_amount"], q["currency"])])
    ...
    story += [ship, Spacer(1, 4.5*mm)]

    payment_methods = (
        "<b>MÉTODOS DE PAGO</b><br/>"
        "• Banco Europa: Transferencia en EUR<br/>"
        "• Banco Estados Unidos: Transferencia en USD<br/>"
        "• Banco Colombia: Transferencia en COP desde cualquier banco de Colombia o desde Casas de Remesas autorizadas"
    )
    payment_box = Table([[Paragraph(payment_methods, small)]], colWidths=[170*mm])
    ...
    story += [payment_box, Spacer(1, 4*mm)]

    cond = (
        "<b>CONDICIONES COMERCIALES</b><br/>"
        "• Esta cotización tiene una vigencia de 15 días calendario contados a partir de su fecha de creación.<br/>"
        "• En modalidad DAP, los impuestos de importación o destino son asumidos y pagados por el cliente directamente a la transportadora. Si lo requiere, Raíz Latina puede cotizar previamente este valor antes de confirmar el pedido.<br/>"
        "• En modalidad DDP, los impuestos indicados en esta cotización están incluidos en el valor total.<br/>"
        "• Los tiempos de entrega son estimados y pueden variar según los tiempos de tránsito, los procesos aduaneros y las condiciones operativas de la transportadora y del país de destino.<br/>"
        "• Este documento corresponde exclusivamente a una cotización. El pedido se procesará una vez recibido el pago correspondiente, para proceder con la preparación y posterior despacho."
    )
    ...
    story += [conditions, Spacer(1, 5*mm), Paragraph("Gracias por elegir Raíz Latina y confiar en nosotros.", ParagraphStyle("RLThanks", parent=small, fontSize=8.5, leading=10, textColor=green, alignment=1))]
    doc.build(story)
    return buf.getvalue()
...
def render_quotations():
    ...
    with tab1:
        ...
        if clients.empty:
            ...
            client_options = clients["id"].tolist()
            sel = st.selectbox(
                "Cliente",
                client_options,
                format_func=lambda x: f"{clients.loc[clients.id==x,'name'].iloc[0]} — {clients.loc[clients.id==x,'country'].iloc[0] or 'Sin país'}",
                key="new_quote_client_select",
                help="Escribe directamente en este campo para buscar por nombre, empresa o país."
            )
            c = q_client(sel)
            a, b = st.columns(2)
            with a:
    ...
    with tab2:
        ...
        if df.empty:
            ...
            st.caption("Cada cotización tiene sus acciones directamente al frente: descargar PDF o editar.")
            header = st.columns([1.2, 1.1, 2.5, 1.5, 1.2, 1.4, 1.5, 2.2])
            for h, label in zip(header, ["Cotización","Fecha","Cliente","País","Total","Estado","Descargar","Editar"]):
                h.markdown(f"**{label}**")
            st.divider()

            for _, row in df.iterrows():
                qid = int(row["id"])
                cols = st.columns([1.2, 1.1, 2.5, 1.5, 1.2, 1.4, 1.5, 2.2])
                cols[0].write(row["Cotización"])
                cols[1].write(row["Fecha"])
                cols[2].write(row["Cliente"])
                cols[3].write(row["País"])
                cols[4].write(q_money(float(row["Total"]), row["Divisa"]))
                cols[5].write(row["Estado"])
                ...
                safe = re.sub(r"[^A-Za-z0-9ÁÉÍÓÚáéíóúÑñ _-]+", "", (client or {}).get("name") or "cliente").strip().replace(" ", "_")
                pdf = q_pdf(q, client) if REPORTLAB_OK and client else None
                if pdf:
                    cols[6].download_button("⬇️ Descargar", pdf, f"Cotizacion {q['quotation_number']} - {safe}.pdf", "application/pdf", key=f"pdf_action_{qid}", use_container_width=True)
                else:
                    cols[6].button("⬇️ Descargar", disabled=True, key=f"pdf_disabled_{qid}", use_container_width=True)

                if cols[7].button("✏️ Editar", key=f"edit_action_{qid}", use_container_width=True):
                    st.session_state[f"edit_quote_{qid}"] = True
                    st.rerun()

                if st.session_state.get(f"edit_quote_{qid}"):
...
if page == "Dashboard":
    ...
    c6.metric("Resultado estimado", fmt_cop(estimated_result))
    c7.metric("Utilidad estimada", f"{estimated_margin:.2f}%")
    ...
    pending = conn.execute("""
        ...
        WHERE status IN ('PAGO PARCIAL','POR CONFIRMAR','CONFIRMADO','MERCANCÍA SOLICITADA','LISTO PARA ENVÍO')
    """).fetchone()[0]
    ...
    if df.empty:
        ...
        if filtered.empty:
            ...
            edited = st.data_editor(
                ...
                disabled=["Pedido", "Cliente", "País", "Fecha", "Valor productos", "Impuestos", "Observación", "Venta total", "Envío real"],
                column_config={
...
# ---------------- SHIPPING ----------------
    ...
    df = read_sql_query("""
        SELECT o.id, o.order_number AS "Pedido", c.name AS "Cliente", o.country AS "País",
               o.order_date AS "Fecha", o.status AS "Estado", o.notes AS "Observación"
        FROM orders o JOIN clients c ON c.id=o.client_id
        WHERE o.status IN ('PAGO PARCIAL','POR CONFIRMAR','CONFIRMADO','MERCANCÍA SOLICITADA','LISTO PARA ENVÍO')
    ...
    if df.empty:
        ...
        h1, h2, h3, h4, h5, h6 = st.columns([1.4, 2.3, 1.4, 1.1, 2.1, 1.6])
        for h, text in zip((h1,h2,h3,h4,h5,h6),("Pedido","Cliente","País","Fecha","Estado","Observaciones")):
            h.markdown(f"**{text}**")
        st.divider()
        ...
        for _, row in df.iterrows():
            ...
            c1, c2, c3, c4, c5, c6 = st.columns([1.4, 2.3, 1.4, 1.1, 2.1, 1.6])
            ...
            if new_status != row["Estado"]:
                ...
                conn.commit()
                st.rerun()
            if c6.button("👁️ Ver Observaciones", key=f"view_obs_{int(row['id'])}", use_container_width=True):
                st.session_state[f"show_obs_{int(row['id'])}"] = not st.session_state.get(f"show_obs_{int(row['id'])}", False)
            if st.session_state.get(f"show_obs_{int(row['id'])}", False):
                obs = str(row["Observación"] or "").strip()
                c6.info(obs if obs else "Sin observaciones.")
