...
if page == "Dashboard":
    ...
    # ESTIMATED shipping cost = 100% of shipping charged.
    total_shipping_cost = conn.execute("""
        SELECT COALESCE(SUM(shipping_sale_cop),0)
        FROM orders
    """).fetchone()[0]
    ...
    st.caption(
        "Resultado estimado = ventas acumuladas − costo de productos − "
        "costo de envío − gastos registrados. Para esta estimación, "
        "el costo de envío es el 100% del envío cobrado al cliente."
    )
    ...
    pending = conn.execute("""
        ...
        WHERE status IN ('POR CONFIRMAR','MERCANCÍA SOLICITADA','LISTO PARA ENVÍO')
    """).fetchone()[0]
...
# ---------------- NEW ORDER ----------------
    ...
    if clients.empty:
        ...
        with st.form("new_order"):
            order_number = st.text_input("Número de pedido", value=next_order_number())
            ...
            st.caption(
                "Todo pedido que se registra aquí se considera pagado. "
                "Si el cliente pagó con varias divisas, puedes registrar el primer pago aquí "
                "y luego agregar movimientos adicionales desde la sección de pagos del pedido."
            )
        ...
        if submitted:
            ...
            try:
                ...
                st.info(f"Estado inicial: POR CONFIRMAR → aparece automáticamente en Envíos pendientes.")
            except sqlite3.IntegrityError:
                st.error("Ese número de pedido ya existe.")
...
# ---------------- ORDERS ----------------
    ...
    st.caption(
        ...
        "una sola vez para registrar el costo real."
    )
    ...
    if df.empty:
        ...
        if filtered.empty:
            ...
            edited = st.data_editor(
                ...
                disabled=["Pedido", "Cliente", "País", "Fecha", "Envío real"],
                column_config={
...
# ---------------- SHIPPING ----------------
    ...
    st.caption(
        "Aquí sólo aparecen pedidos que todavía no están ENVIADOS. "
        "El estado se cambia directamente desde la columna Estado."
    )
    ...
    df = pd.read_sql_query("""
        ...
        WHERE o.status IN ('POR CONFIRMAR','MERCANCÍA SOLICITADA','LISTO PARA ENVÍO')
    ...
    if df.empty:
        ...
        # Editable status directly in the table.
        edit_df = df.drop(columns=["id"]).copy()
        status_options = [
            "POR CONFIRMAR",
            "MERCANCÍA SOLICITADA",
            "LISTO PARA ENVÍO",
        ...
        edited = st.data_editor(
            ...
            column_config={
                ...
                "País": st.column_config.TextColumn(disabled=True),
            ...
            disabled=["Pedido", "Cliente", "País", "Fecha"],
            num_rows="fixed"
        )

        # Detect status changes and persist them automatically.
        for i in range(len(edited)):
            original_status = df.iloc[i]["Estado"]
            new_status = edited.iloc[i]["Estado"]
            if new_status != original_status:
                oid = int(df.iloc[i]["id"])
